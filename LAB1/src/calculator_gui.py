        ...
        for button in buttons:
            if button == '=':
                ...
                btn = tk.Button(self.root, text=button, font=('Arial', 14),
                                command=cmd, bg='#4CAF50', fg='white', height=2, width=5)
            elif button == 'C':
                cmd = lambda: self.clear()
                btn = tk.Button(self.root, text=button, font=('Arial', 14),
                                command=cmd, bg='#F44336', fg='white', height=2, width=5)
            elif button == '⌫':
                cmd = lambda: self.backspace()
                btn = tk.Button(self.root, text=button, font=('Arial', 14),
                                command=cmd, bg='#FF9800', fg='white', height=2, width=5)
            elif button in ['+', '-', '*', '/']:
                cmd = lambda x=button: self.button_click(x)
                btn = tk.Button(self.root, text=button, font=('Arial', 14),
                                command=cmd, bg='#2196F3', fg='white', height=2, width=5)
            else:
                cmd = lambda x=button: self.button_click(x)
                btn = tk.Button(self.root, text=button, font=('Arial', 14),
                                command=cmd, bg='#E0E0E0', fg='black', height=2, width=5)
